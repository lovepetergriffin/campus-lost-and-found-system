...
根据 S4 的 CK、LK 与 MOOD 度量结果，后端已完成按职责拆分：

- 物品模块拆为查询、命令、状态流转、查询条件构造、访问策略和视图装配，避免单个服务同时承担筛选、维护、权限与 DTO 转换。
- 认领模块拆为申请服务、审批服务和视图装配；复杂审批方法进一步提取校验、状态更新、批量拒绝及通知步骤。
- 管理模块拆为审核服务和统计服务，审核流程不再直接承担全系统聚合统计。
- REST API 路径和响应结构保持不变，前端无需因本次重构调整调用方式。
...
后端集成测试覆盖“注册 → 发布 → 审核 → 查询 → 认领 → 确认 → 通知 → 统计”的完整主流程，以及重复认领、越权审批和未授权访问拦截。
